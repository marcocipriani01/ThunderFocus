...
void saveSettings() {
	...
	settings.reverseDir = focuser.getDirReverse();
#endif
#if ENABLE_DEVMAN == true
	for (uint8_t i = 0; i < getPwmPinCount(); i++) {
		settings.devsPwmStates[i] = getPwmPinValue(i);
	}
	for (uint8_t i = 0; i < getDioPinCount(); i++) {
		settings.devsDioStates[i] = getDioPinValue(i);
	}
#endif
	uint8_t* dst = (uint8_t*)&settings;
	for (unsigned int i = 0; i < sizeof(Settings); ++i) {
...
void setup() {
	...
	if (settingsOk) {
		for (uint8_t i = 0; i < getPwmPinCount(); i++) {
			updatePin(getPwmPin(i), settings.devsPwmStates[i]);
		}
		for (uint8_t i = 0; i < getDioPinCount(); i++) {
			updatePin(getDioPin(i), settings.devsDioStates[i] ? 255 : 0);
		}
	}
#endif
